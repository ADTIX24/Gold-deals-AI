          ...
          <div className="bg-white/5 backdrop-blur-xl p-6 rounded-2xl border border-white/5 space-y-6">
            ...
            <div className="space-y-6">
                        ...
                        if (file) {
                          ...
                          reader.onload = (ev) => {
                            ...
                            img.onload = () => {
                              ...
                              const MAX_SIZE = 120; // 120px max logo dimension ensures tiny file size
                              let width = img.width;
                              ...
                              if (ctx) {
                                ...
                                // Output compressed JPEG which is extremely small (few KB)
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {siteConfig.logoUrl && (
                        <button
                          ...
                          className="px-4 py-2 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 text-rose-400 text-[10px] font-black rounded-lg transition-all active:scale-95 uppercase tracking-widest"
                        >
              ...
              <div className="space-y-4">
                {/* Market Status (Sentiment) Control */}
                <div className="p-4 bg-slate-900/40 rounded-2xl border border-white/5 space-y-4">
                  <label className="text-[10px] text-slate-500 font-bold uppercase tracking-widest block">{isRtl ? 'السوق' : 'Market Status'}</label>
                  <div className="grid grid-cols-3 bg-slate-950/50 p-1.5 rounded-xl border border-white/5 gap-1.5">
                    <button
                      type="button"
                      onClick={() => updateSetting('marketSentiment', 'safe')}
                      className={cn(
                        "py-2 px-3 rounded-lg text-xs font-bold transition-all flex flex-col md:flex-row items-center justify-center gap-1.5",
                        siteConfig.marketSentiment === 'safe' || !siteConfig.marketSentiment
                    ...
                    <button
                      type="button"
                      onClick={() => updateSetting('marketSentiment', 'tense')}
                      className={cn(
                        "py-2 px-3 rounded-lg text-xs font-bold transition-all flex flex-col md:flex-row items-center justify-center gap-1.5",
                        siteConfig.marketSentiment === 'tense'
                          ? "bg-amber-500/15 text-amber-400 border border-amber-500/20"
                          : "text-slate-500 hover:text-slate-300 border border-transparent"
                      )}
                    >
                      <div className="w-2.5 h-2.5 rounded-full bg-amber-500 animate-pulse shrink-0" />
                      <span>{isRtl ? 'متوتر' : 'Tense'}</span>
                    </button>
                    <button
                      type="button"
                      onClick={() => updateSetting('marketSentiment', 'dangerous')}
                      className={cn(
                        "py-2 px-3 rounded-lg text-xs font-bold transition-all flex flex-col md:flex-row items-center justify-center gap-1.5",
                        siteConfig.marketSentiment === 'dangerous'
                          ? "bg-rose-500/15 text-rose-400 border border-rose-500/20"
                          : "text-slate-500 hover:text-slate-300 border border-transparent"
                      )}
                    >
                      <div className="w-2.5 h-2.5 rounded-full bg-rose-500 animate-pulse shrink-0" />
                      <span>{isRtl ? 'خطير' : 'Dangerous'}</span>
                    </button>
                  </div>
                </div>
                ...
                <div className="p-4 bg-slate-900/40 rounded-2xl border border-white/5 space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <SettingInput label="WhatsApp" value={siteConfig.whatsappUrl} onChange={(v) => updateSetting('whatsappUrl', v)} />
                    <SettingInput label="Facebook" value={siteConfig.facebookUrl} onChange={(v) => updateSetting('facebookUrl', v)} />
...
const AIAnalyst = ({ userProfile, isRtl }: any) => {
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem(`ai_cooldown_timestamp_${userProfile.uid}`);
    if (stored) {
      const lastTime = parseInt(stored, 10);
      const elapsedMs = Date.now() - lastTime;
      const cooldownDurationMs = 900 * 1000; // 15 minutes
      if (elapsedMs < cooldownDurationMs) {
        const elapsedSecs = Math.floor(elapsedMs / 1000);
        setCooldownTime(elapsedSecs);
    ...
    <div className={cn("flex flex-col min-h-screen selection:bg-cyan-500/30 selection:text-white bg-[#050505] text-slate-200", isRtl ? "font-sans-arabic" : "font-sans")}>
            ...
            <aside className="w-full xl:w-96 space-y-6">
              ...
              <div className="bg-white/5 border border-white/5 p-4 rounded-xl flex items-center justify-between">
                <span className="text-[11px] font-bold uppercase text-slate-400 tracking-wider">
                  {isRtl ? 'السوق' : 'Market Status'}
                </span>
                <div className="flex items-center space-x-3 rtl:space-x-reverse">
                  <span className={cn(
                    "text-[11px] font-black px-2.5 py-0.5 rounded-full text-center tracking-wide",
                    (siteSettings.marketSentiment === 'safe' || !siteSettings.marketSentiment) ? "text-emerald-400 bg-emerald-500/10" :
                    siteSettings.marketSentiment === 'tense' ? "text-amber-400 bg-amber-500/10" :
                    "text-rose-400 bg-rose-500/10"
                  )}>
                    {(siteSettings.marketSentiment === 'safe' || !siteSettings.marketSentiment) && (isRtl ? 'آمن' : 'Safe')}
                    {siteSettings.marketSentiment === 'tense' && (isRtl ? 'متوتر' : 'Tense')}
                    {siteSettings.marketSentiment === 'dangerous' && (isRtl ? 'خطير' : 'Dangerous')}
                  </span>
                  <div className="flex items-center gap-1.5 [direction:ltr]">
                    <div className={cn(
                      "w-6 h-1.5 rounded-full transition-all duration-300",
                      (siteSettings.marketSentiment === 'safe' || !siteSettings.marketSentiment) ? "bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]" : "bg-slate-800"
                    )} />
                    <div className={cn(
                      "w-6 h-1.5 rounded-full transition-all duration-300",
                      siteSettings.marketSentiment === 'tense' ? "bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.5)]" : "bg-slate-800"
                    )} />
                    <div className={cn(
                      "w-6 h-1.5 rounded-full transition-all duration-300",
                      siteSettings.marketSentiment === 'dangerous' ? "bg-rose-500 shadow-[0_0_10px_rgba(244,63,94,0.5)]" : "bg-slate-800"
                    )} />
                  </div>
                </div>
              </div>
      ...
      <footer className="mt-auto bg-slate-950/80 backdrop-blur-xl border-t border-white/5">
        <div className="max-w-[1600px] mx-auto px-4 md:px-8 py-12 flex flex-col items-center justify-center gap-10">
          ...
          {/* Middle: Social */}
          <div className="flex flex-wrap items-center justify-center gap-6 p-4 bg-white/5 rounded-2xl border border-white/5">
            <SocialIcon icon={<Send size={20} />} href={siteSettings.telegramUrl} color="hover:text-[#229ED9]" />
            <SocialIcon icon={<MessageCircle size={20} />} href={siteSettings.whatsappUrl} color="hover:text-emerald-500 hover:shadow-[0_0_15px_rgba(16,185,129,0.3)]" />
            <SocialIcon icon={<Facebook size={20} />} href={siteSettings.facebookUrl} color="hover:text-blue-500 hover:shadow-[0_0_15px_rgba(59,130,246,0.3)]" />
